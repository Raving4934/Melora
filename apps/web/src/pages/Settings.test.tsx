...
describe('v5 Settings 字段自动保存界面', () => {
  ...
  it('源使用 Hi-Res 别名时仍提供统一的 24bit 选项', async () => {
    sources.items[0].platforms.wy.qualitys = ['128k', 'hires']
    show()
    await ready()
    const quality = screen.getByLabelText('在线播放音质')
    expect(within(quality).getByRole('option', { name: 'Hi-Res · 24bit' })).toHaveValue('flac24bit')
  })
