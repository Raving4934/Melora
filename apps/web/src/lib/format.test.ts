...
it('真实有损格式显示编码与码率，不伪装MP3档位', () => {
  expect(qualityName('aac100k')).toBe('AAC · 100 kbps')
  expect(qualityName('ogg96k')).toBe('OGG · 96 kbps')
  expect(qualityName('64k')).toBe('MP3 · 64 kbps')
  expect(qualityName('128k')).toBe('标准 · 128 kbps')
})
