...
import type { PlayInfo, Provider, Track } from '../lib/types'
...
describe('无源播放保护', () => {
  it('已确认无播放能力时不请求在线地址，也不改变当前队列和播放', async () => {
    await player.play(track)
    const previousURL = currentAudio!.src
    const previousQueue = usePlayer.getState().queue
    requests.length = 0

    queryClient.setQueryData(
      ['/providers'],
      [{ ...liveProvider, capabilities: { ...liveProvider.capabilities, play: false } }],
    )
    await expect(player.play(liveTrack, [liveTrack])).resolves.toBeUndefined()

    expect(usePlayer.getState()).toMatchObject({ track, queue: previousQueue, playing: true, loading: false })
    expect(currentAudio!.src).toBe(previousURL)
    expect(currentAudio!.paused).toBe(false)
    expect(playedRequests()).toHaveLength(0)
    expect(useUI.getState().sourceGate).toBe('play')
  })

  it('后端返回结构化无源错误时保留当前媒体、队列和播放状态', async () => {
    await player.play(track)
    const previousURL = currentAudio!.src
    const previousQueue = usePlayer.getState().queue
    requests.length = 0
    queryClient.setQueryData(['/providers'], [liveProvider])
    resolve = () => json({ error: { code: 'lx_source_required', message: '尚未选择可用的 LX 音源' } }, 409)
  ...
  it.each(['track', 'quality'] as const)(
    '%s 解析无源只撤销暂存字段，保留用户设置、队列编辑与旧媒体真实进度',
    async (reason) => {
      const removed = { ...track, id: 'demo:remove' }
      const remaining = { ...track, id: 'demo:remain' }
      const added = { ...track, id: 'demo:added' }
      await player.playCollection('original-list', [track, removed, remaining])
      await vi.waitFor(() => expect(usePlayer.getState().historyRecorded).toBe(true))
      player.seek(11)
      const original = usePlayer.getState()
      const previousURL = currentAudio!.src
      const loadCalls = currentAudio!.load.mock.calls.length
      const playCalls = currentAudio!.play.mock.calls.length
      queryClient.setQueryData(['/providers'], [liveProvider])
      ...
      const pending =
        reason === 'track' ? player.playCollection('pending-list', [liveTrack]) : player.setQuality('flac')
      await vi.waitFor(() => expect(release).toBeTypeOf('function'))

      // 候选队列不对外发布，追加/移除编辑的仍是用户原来的队列。
      expect(usePlayer.getState().queue).toBe(original.queue)
      player.remove(track.id)
      expect(usePlayer.getState().queue).toBe(original.queue)
      player.remove(removed.id)
      // 与 TrackList「加入队列」相同的公开 store 写入，不能靠只拦 player 方法保护。
      usePlayer.setState((state) => ({ queue: [...state.queue, added] }))
      const editedQueue = usePlayer.getState().queue
      player.volume(0.34)
      player.toggleMute()
      player.setRate(1.5)
      player.cycleMode()
      usePlayer.setState({ autoSwitchSource: false })
      currentAudio!.currentTime = 37
      currentAudio!.duration = 102
      currentAudio!.dispatchEvent(new Event('timeupdate'))
      currentAudio!.dispatchEvent(new Event('durationchange'))

      release(json({ error: { code: 'lx_source_required', message: '需要自定义源' } }, 409))
      await expect(pending).resolves.toBeUndefined()
      expect(usePlayer.getState()).toMatchObject({
        track,
        queue: [track, remaining, added],
        playing: true,
        loading: false,
        recovering: false,
        error: null,
        ...
        resolvedQuality: original.resolvedQuality,
        sourceId: original.sourceId,
        attemptedSources: original.attemptedSources,
        resolvedTrack: original.resolvedTrack,
        availableQualities: original.availableQualities,
      })
      expect(usePlayer.getState().queue).toBe(editedQueue)
      expect(usePlayingCollection.getState().collectionId).toBe('original-list')
      ...
      expect(currentAudio!.load).toHaveBeenCalledTimes(loadCalls)
      expect(currentAudio!.play).toHaveBeenCalledTimes(playCalls)
      // 恢复后的持久化也不能写回旧偏好和旧队列。
      player.pause()
      ...
      player.toggleMute()
      expect(usePlayer.getState()).toMatchObject({ volume: 0.34, muted: false })
      expect(currentAudio).toMatchObject({ volume: 0.34, muted: false })
    },
  )

  it('解析期间清空待播后再追加歌曲，无源失败不复活被清空项或候选队列', async () => {
    const removed = { ...track, id: 'demo:clear' }
    const added = { ...track, id: 'demo:after-clear' }
    await player.play(track, [track, removed])
    let release!: (value: Response) => void
    resolve = () =>
      new Promise<Response>((done) => {
        release = done
      })
    const pending = player.play(liveTrack, [liveTrack])
    await vi.waitFor(() => expect(release).toBeTypeOf('function'))
    player.clearQueue()
    ...
    expect(usePlayer.getState()).toMatchObject({ track, queue: [track, added], playing: true })
    expect(usePlayer.getState().queue).toBe(editedQueue)
    expect(currentAudio!.paused).toBe(false)
  })

  it.each(['resolved', 'failed'] as const)(
    '并发队列编辑优先于候选整队替换，%s 时只补入选中曲目',
    async (result) => {
      const originalNext = { ...track, id: 'demo:original-next' }
      const added = { ...track, id: 'demo:queued-during-resolve' }
      const candidateNext = { ...liveTrack, id: 'wy:candidate-next' }
      await player.play(track, [track, originalNext])
      let release!: (value: PlayInfo | Response) => void
      resolve = () =>
        new Promise<PlayInfo | Response>((done) => {
          release = done
        })
      const pending = player.play(liveTrack, [liveTrack, candidateNext])
      await vi.waitFor(() => expect(release).toBeTypeOf('function'))
      expect(usePlayer.getState().queue).toEqual([track, originalNext])
      usePlayer.setState((state) => ({ queue: [...state.queue, added] }))
      release(
        result === 'resolved'
          ? { ...info, trackId: liveTrack.id }
          : json({ error: { code: 'invalid_media_result', message: '媒体无效' } }, 502),
      )
      await expect(pending).resolves.toBeUndefined()

      expect(usePlayer.getState()).toMatchObject({
        track: liveTrack,
        queue: [liveTrack, track, originalNext, added],
        playing: result === 'resolved',
        error: result === 'resolved' ? null : '媒体无效',
      })
      expect(useUI.getState().sourceGate).toBeNull()
    },
  )

  it('旧媒体在解析期间自然结束时，源错误只恢复曲目与媒体地址，不伪造继续播放', async () => {
    await player.play(track)
    const previousURL = currentAudio!.src
    const previousQueue = usePlayer.getState().queue
    requests.length = 0
    ...
    currentAudio!.end()
    release(json({ error: { code: 'lx_source_required', message: '尚未选择可用的 LX 音源' } }, 409))
    await pending
    ...
    // 只靠解析期间的那一次真实 ended 结算，不人工再补一次 ended。
    await vi.waitFor(() =>
      expect(requests.filter(({ url }) => url.pathname.endsWith('/outcome'))).toHaveLength(1),
    )
    const outcome = requests.find(({ url }) => url.pathname.endsWith('/outcome'))!
    expect(outcome.url.pathname).toBe('/api/v1/library/history/demo%3Aone/outcome')
    expect(outcome.init?.method).toBe('POST')
    expect(JSON.parse(String(outcome.init?.body))).toEqual({
      eventId: expect.any(String),
      dataGeneration: 'a'.repeat(32),
      playedMs: expect.any(Number),
      completed: true,
    })
    expect(playedRequests()).toHaveLength(1)
    expect(requests.filter(({ url }) => url.pathname.endsWith('/history'))).toHaveLength(0)
  })

  it('源错误回滚会恢复旧播放结局计时，不把解析期间的暂停区间算给旧歌', async () => {
    ...
    try {
      ...
      currentAudio!.dispatchEvent(new Event('playing'))
      nowSpy.mockReturnValue(14_000)
      release(json({ error: { code: 'lx_source_required', message: '尚未选择可用的 LX 音源' } }, 409))
      await pending

      expect(usePlayer.getState()).toMatchObject({ track, playing: true, loading: false })
      expect(currentAudio!.src).toBe(previousURL)
      expect(currentAudio!.paused).toBe(false)
  ...
  it('无源回滚时旧媒体仍在缓冲，不把回滚到真实 playing 之间的时间计入收听', async () => {
    const nowSpy = vi.spyOn(Date, 'now').mockReturnValue(10_000)
    try {
      await player.play(track)
      requests.length = 0
      ...
      const pending = player.play(liveTrack)
      await vi.waitFor(() => expect(release).toBeTypeOf('function'))
      nowSpy.mockReturnValue(12_000)
      currentAudio!.dispatchEvent(new Event('waiting'))
      nowSpy.mockReturnValue(13_000)
  ...
  it('invalid_media_result 不使用无源回滚，停止旧媒体且只结算旧歌曲实际收听', async () => {
    const nowSpy = vi.spyOn(Date, 'now').mockReturnValue(10_000)
    try {
      await player.play(track)
      await vi.waitFor(() => expect(usePlayer.getState().historyRecorded).toBe(true))
      requests.length = 0
      nowSpy.mockReturnValue(12_000)
      resolve = () => json({ error: { code: 'invalid_media_result', message: '音源脚本返回结果无效' } }, 502)
      await player.play(liveTrack)
      expect(usePlayer.getState()).toMatchObject({
        track: liveTrack,
        queue: [liveTrack],
        playing: false,
      ...
      expect(currentAudio).toMatchObject({ src: '', paused: true })
      expect(useUI.getState().sourceGate).toBeNull()
      await vi.waitFor(() =>
        expect(requests.filter(({ url }) => url.pathname.endsWith('/outcome'))).toHaveLength(1),
      )
  ...
  it('用户在解析期间暂停会取消新请求，不被迟到的无源错误恢复为播放', async () => {
    await player.play(track)
    queryClient.setQueryData(['/providers'], [liveProvider])
    let release!: (value: Response) => void
    resolve = () =>
...
describe('默认音质、倍速与音质重解析', () => {
  it('保留实际AAC编码与码率，但不把返回描述加入可请求音质菜单', async () => {
    resolve = () => ({ ...info, quality: 'aac100k', mimeType: 'audio/aac' })
    await player.play(track)
    expect(usePlayer.getState()).toMatchObject({
      playing: true,
      requestedQuality: 'standard',
      resolvedQuality: 'aac100k',
    })
    expect(usePlayer.getState().availableQualities).not.toContain('aac100k')
    expect(playedRequests()).toHaveLength(1)
  })
  ...
  it.each(['pause', 'end'] as const)(
    '音质解析期间旧媒体 %s 后，解析失败恢复原音质但不重新播放',
    async (event) => {
      await player.play(track)
      player.seek(19)
      const playCalls = currentAudio!.play.mock.calls.length
      ...
      const pending = player.setQuality('flac')
      await vi.waitFor(() => expect(release).toBeTypeOf('function'))
      currentAudio![event]()
      release(json({ error: { code: 'lx_resolve_failed', message: '解析失败' } }, 502))
      await expect(pending).resolves.toBeUndefined()
      expect(usePlayer.getState()).toMatchObject({
        track,
        quality: 'standard',
      ...
      expect(usePlayer.getState().error).toContain('保留原音质')
      expect(currentAudio).toMatchObject({ src: info.url, currentTime: 19, paused: true })
      expect(currentAudio!.play).toHaveBeenCalledTimes(playCalls)
    },
  )
...
describe('v7 解析诊断与Promise拒绝边界', () => {
  ...
  it.each(['pause', 'load'] as const)(
    '新地址提交阶段 Audio.%s 抛出 undefined 仍显示初始化错误并停止真实媒体',
    async (method) => {
      await player.play(track)
      requests.length = 0
      currentAudio![method].mockImplementationOnce(() => {
        throw undefined
      })
      await expect(player.play(liveTrack)).resolves.toBeUndefined()
      expect(usePlayer.getState()).toMatchObject({ track: liveTrack, playing: false, loading: false })
  ...
  it('没有旧媒体时 load 抛出 undefined 也清理已写入的新地址', async () => {
    player.volume(0.7)
...
it('切换音质失败后恢复旧Audio再次抛出undefined也不会漏出拒绝', async () => {
  ...
  currentAudio!.load.mockClear()
  const playCalls = currentAudio!.play.mock.calls.length
  // 新解析之前已不再 load 空地址；第一次 load 就是解析失败后的旧媒体恢复。
  currentAudio!.load.mockImplementationOnce(() => {
    throw undefined
  })
  ...
  expect(currentAudio).toMatchObject({ src: '', paused: true })
  // 一次恢复失败，一次无地址清理；不能重新调用 play 让旧媒体继续发声。
  expect(currentAudio!.load).toHaveBeenCalledTimes(2)
  expect(currentAudio!.play).toHaveBeenCalledTimes(playCalls)
  const requestCount = playedRequests().length
  currentAudio!.dispatchEvent(new Event('playing'))
  currentAudio!.fail()
  expect(usePlayer.getState().playing).toBe(false)
  expect(playedRequests()).toHaveLength(requestCount)
})
