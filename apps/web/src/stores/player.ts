...
import { guardOnlineOperation, handleSourceRequiredError } from '../lib/source-state'
...
type ResolveResult = 'resolved' | 'source-required' | 'failed' | 'cancelled'
type ResolveTransaction = {
  ...
  previousDesiredPlaying: boolean
  previousOutcome: PlayOutcomeSession | null
  oldMediaInterrupted: boolean
  committing: boolean
}
let activeResolveTransaction: ResolveTransaction | undefined
...
function flushPlayOutcome(completed: boolean, session = playOutcomeSession) {
...
function oldMediaTransaction(element: HTMLAudioElement) {
  const transaction = activeResolveTransaction
  if (
    !transaction ||
    transaction.version !== requestVersion ||
    transaction.committing ||
    !transaction.previousMediaActive ||
    transaction.element !== element ||
    element.src !== transaction.previousURL
  )
    return undefined
  return transaction
}
// 解析尚未提交时，队列仍属于原当前曲目；移除/清空不能把待解析曲目当成已选中。
function currentQueueTrack(state: PlayerState) {
  const transaction = activeResolveTransaction
  return resolving && transaction?.version === requestVersion && !transaction.committing
    ? transaction.previousTrack
    : state.track
}
...
// 异常收尾必须同时撤销媒体所有权和实际音频，不能只把 store 标成暂停。
function discardMediaAfterFailure(element: HTMLAudioElement | undefined) {
  desiredPlaying = false
  expectedURL = ''
  mediaVersion = 0
  ...
  try {
    ...
    // pause 本身也可能抛出；仍需解除媒体地址。
  }
  try {
    element.removeAttribute('src')
    element.load()
  } catch {
    // 清理失败不遮蔽原始错误，已撤销的媒体事件也不能重启播放器。
  }
}
...
function getAudio() {
  ...
  element.addEventListener('playing', () => {
    const transaction = oldMediaTransaction(element)
    if (transaction) {
      // 解析时仍在响的是旧媒体；缓冲恢复只接续旧会话，不能给待解析的新歌记历史。
      const session = transaction.previousOutcome
      if (session && !session.sent) startPlayClock(session)
      return
    }
  ...
  element.addEventListener('pause', () => {
    const transaction = oldMediaTransaction(element)
    if (transaction) transaction.oldMediaInterrupted = true
    stopPlayClock()
  ...
  element.addEventListener('ended', () => {
    const transaction = oldMediaTransaction(element)
    if (transaction) {
      transaction.oldMediaInterrupted = true
      clearStallTimer()
      flushPlayOutcome(true, transaction.previousOutcome)
      return
    }
    if (!activeMedia()) return
    clearStallTimer()
...
async function resolveTrack(track: Track, options: ResolveOptions): Promise<ResolveResult> {
  if (!options.sourceGuarded && !guardOnlineOperation(track, 'play')) return 'source-required'
  const snapshot = usePlayer.getState()
  const previousURL = expectedURL
  const previousMediaActive = activeMedia()
  const previousDesiredPlaying = desiredPlaying
  const rollbackState = options.rollbackState ?? snapshot
  const version = ++requestVersion
  const transaction: ResolveTransaction = {
    version,
    previousTrack: rollbackState.track,
    commitSelection: options.commitSelection,
    element: audio,
    previousURL,
    previousMediaActive,
    previousDesiredPlaying,
    previousOutcome: playOutcomeSession,
    oldMediaInterrupted: false,
    committing: false,
  }
  activeResolveTransaction = transaction
  ...
  try {
    element = getAudio()
    // 保留当前媒体直到新地址通过校验；无源/后端保底失败时可无缝回到原播放。
    phase = 'resolve'
    ...
    set({
      ...
      resolvedQuality:
        typeof info.quality === 'string' && /^[a-z0-9_-]{1,32}$/i.test(info.quality) ? info.quality : null,
    ...
    if (handleSourceRequiredError(error, 'play')) {
      clearStallTimer()
      playOutcomeSession = transaction.previousOutcome
      const restorePlaying =
        previousMediaActive &&
        previousDesiredPlaying &&
        !transaction.oldMediaInterrupted &&
        rollbackState.playing &&
        !!transaction.element &&
        !transaction.element.paused &&
        !transaction.element.ended
      desiredPlaying = restorePlaying
      expectedURL = previousMediaActive ? previousURL : ''
      mediaVersion = previousMediaActive ? version : 0
      // 旧媒体没有重新 load，不需要重新 seek；保留它当前真实进度。
      pendingSeek = undefined
      // 只撤销本次解析暂存的字段；队列和用户偏好从未归本事务所有。
      const previousMedia = previousMediaActive ? transaction.element : undefined
      set({
        track: rollbackState.track,
        resolvedTrack: rollbackState.resolvedTrack,
        requestedQuality: rollbackState.requestedQuality,
        resolvedQuality: rollbackState.resolvedQuality,
        availableQualities: rollbackState.availableQualities,
        sourceId: rollbackState.sourceId,
        attemptedSources: rollbackState.attemptedSources,
        error: rollbackState.error,
        playing: restorePlaying,
        loading: restorePlaying && rollbackState.loading,
        recovering: restorePlaying && rollbackState.recovering,
        position:
          previousMedia && Number.isFinite(previousMedia.currentTime)
            ? previousMedia.currentTime
            : rollbackState.position,
        duration:
          previousMedia && Number.isFinite(previousMedia.duration) && previousMedia.duration > 0
            ? previousMedia.duration
            : rollbackState.duration,
      })
      // 计时只由真实 playing/waiting/pause 事件驱动；paused=false 也可能仍在缓冲。
      if (restorePlaying) armStallTimer()
      // 统一由源引导弹层呈现；不要清队列，也不要销毁仍可用的当前媒体。
      return 'source-required'
    }
    // 切换音质失败可恢复同一首歌的旧媒体；不能把失败的新音质标成成功。
    if (element && options.reason === 'quality' && previousURL && snapshot.track?.id === track.id) {
      transaction.committing = true
      desiredPlaying = previousDesiredPlaying && !transaction.oldMediaInterrupted
      ...
      try {
        ...
        discardMediaAfterFailure(element)
        failPlayback('播放器无法恢复媒体，请刷新页面重试。')
      }
    } else {
      // 普通失败沿用旧行为：新解析失败时停止旧媒体；只有源必需错误才保留当前播放。
      discardMediaAfterFailure(element)
      failPlayback(message)
      // 普通失败保留选中失败曲目的既有行为；无源错误不提交选择。
      options.commitSelection?.()
    }
    return 'failed'
  } finally {
    ...
    if (activeResolveTransaction === transaction) activeResolveTransaction = undefined
  }
}
...
export const player = {
  ...
  // 集合播放的统一入口：先捕获旧上下文，再提交新集合；失败时由 player.play 回滚。
  playCollection(collectionId: string, tracks: Track[], start?: Track) {
    ...
    if (!guardOnlineOperation(track, 'batch')) return Promise.resolve()
    // 让 player.play 在解析开始前捕获旧集合；历史分类会在异步解析完成前读取新上下文。
    const pending = player.play(track, tracks, { preserveCollection: true })
    usePlayingCollection.getState().setCollection(collectionId, tracks)
    return pending
  },
  play(track: Track, queue?: Track[], options: PlayOptions = {}) {
    if (!guardOnlineOperation(track, 'play')) return Promise.resolve()
    const previousState = usePlayer.getState()
    const previousCollection = usePlayingCollection.getState()
    recoveryCount = 0
    legacyRefreshCount = 0
    excludedSources = []
    ...
    const nextQueue = queue?.length
      ...
      : existing.some((item) => item.id === track.id)
        ? existing
        : [track]
    const selectedQueue = [...new Map(nextQueue.map((item) => [item.id, item])).values()]
    ...
    return resolveTrack(track, {
      ...
      commitSelection: () => {
        ...
        set((state) => ({
          // 候选队列不提前发布；期间若用户编辑过队列，以用户的新队列为准。
          queue:
      ...
      if (result === 'source-required') usePlayingCollection.setState(previousCollection)
    })
  },
  pause() {
    desiredPlaying = false
    ...
    if (resolving) {
      const transaction = activeResolveTransaction
      // 主动取消仍保留已选曲目；与成功/普通失败共用一次性选择提交。
      if (transaction?.version === requestVersion) transaction.commitSelection?.()
      requestVersion++
      activeController?.abort()
      activeController = undefined
      resolving = false
  ...
  remove(id: string) {
    const state = usePlayer.getState()
    if (currentQueueTrack(state)?.id !== id) set({ queue: state.queue.filter((item) => item.id !== id) })
  },
  clearQueue() {
    const track = currentQueueTrack(usePlayer.getState())
    set({ queue: track ? [track] : [] })
  },
}
