...
import { SOURCE_REQUIRED_MESSAGE, guardOnlineOperation, handleSourceRequiredError } from '../lib/source-state'
...
function DownloadForm({ track }: { track: Track }) {
  ...
  const start = async () => {
    if (pending || details.isFetching) return
    if (!guardOnlineOperation(track, 'download')) {
      setError(SOURCE_REQUIRED_MESSAGE)
      return
    }
    if (capabilityFailed || !details.data?.canDownload || !qualities.length) return
    setPending(true)
    setError('')
    try {
      ...
      setError(handleSourceRequiredError(error, 'download') ? SOURCE_REQUIRED_MESSAGE : errorMessage(error))
    } finally {
      setPending(false)
    }
  }
