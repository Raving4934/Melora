...
describe('目录的真实导航与查询切换', () => {
  ...
  it('发现推荐收到结构化无源错误时打开统一源设置入口', async () => {
    ...
    const fetcher = browseFetch((url) => {
      ...
      if (url.pathname.endsWith('/play-info'))
        return response({ error: { code: 'lx_source_required', message: '尚未选择可用的 LX 音源' } }, 409)
      return response([])
    })
    browseUI(<DiscoverPage />, '/discover')
    fireEvent.click(await screen.findByRole('button', { name: `播放 ${track.title}` }))
    const dialog = await screen.findByRole('dialog', { name: '需要配置自定义源' })
    expect(dialog).toHaveTextContent('请先在设置中导入并启用一个可用的自定义音源。')
    expect(dialog).toHaveTextContent('播放新的在线曲目需要先取得可用的在线地址。')
    expect(within(dialog).getByRole('link', { name: '打开自定义源设置' })).toHaveAttribute(
      'href',
