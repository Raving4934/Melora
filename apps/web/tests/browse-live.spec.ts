...
import type { PlayInfo, Provider, Settings } from '../src/lib/types'
...
// 与现有浏览器播放用例相同的 PCM WAV；只截获媒体响应，不替换 Audio 或播放器状态。
const mediaURL = 'https://media.example.org/browse-fixture.wav'
function silentWav() {
...
async function fixture(page: Page, { playable = true } = {}) {
  const state = {
    details: [] as string[],
    dailyRequests: 0,
    newRequests: 0,
    ...
    failDaily: false,
    failChart: false,
    sourceRequired: !playable,
    providerRequests: 0,
    sourceRequests: 0,
    playInfoRequests: [] as string[],
    mediaRequests: 0,
  }
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path === '/auth/session') body = { authenticated: true, required: false }
      ...
      // 普通用户可见的完整能力合同；download=false 是未配置目录，不代表没有音频源。
      body = providers.map((id): Provider => ({
        id,
        name: names[id]!,
        description: '离线目录与媒体夹具',
        enabled: true,
        isDemo: false,
        status: 'catalog',
        capabilities: {
          ...
          search: true,
          playlists: true,
          recommendations: id === 'wy',
          play: playable,
          download: false,
        },
      }))
    } else if (path === '/sources') {
      // 不能为试听补管理员权限；若生产代码误依赖 sources，测试仍暴露真实拒绝。
      state.sourceRequests++
      await route.fulfill({
        status: 403,
        json: {
          error: { code: 'fnos_admin_required', message: '当前版本使用共享音乐数据，仅允许飞牛管理员访问' },
        },
      })
      return
    ...
    } else if (path.startsWith('/tracks/') && path.endsWith('/play-info')) {
      const trackId = path.slice('/tracks/'.length, -'/play-info'.length)
      state.playInfoRequests.push(trackId)
      if (state.sourceRequired) {
        await route.fulfill({
          status: 409,
      ...
      body = {
        ...
        mimeType: 'audio/wav',
        quality: '128k',
        sourceId: 'browse-fixture',
        attemptedSources: ['browse-fixture'],
      } satisfies PlayInfo
    } else if (path.startsWith('/tracks/') && path.endsWith('/lyrics')) {
      body = { lines: [], source: 'browse-fixture' }
    } else {
...
test('164榜以4个热门榜试听、全量目录检索与平台筛选呈现', async ({ page }) => {
  ...
  expect(state.playInfoRequests).toEqual(['tx:330'])
  expect(state.mediaRequests).toBeGreaterThan(0)
  expect(state.sourceRequests).toBe(0)
  await expect(page.getByRole('dialog', { name: '需要配置自定义源' })).toHaveCount(0)
...
test('发现直接消费daily的18曲12歌单，失败保留内容，次级新歌按需请求', async ({ page }) => {
  ...
  await expect(page.locator('.daily-track-region .track-row')).toHaveCount(18)
  await expect(page.locator('.daily-playlists .collection-item')).toHaveCount(12)
  // capability 缓存仍可用，但解析时源已不可用：验证后端 409 保底，而不是让预检挡住请求。
  state.sourceRequired = true
  await page.getByRole('button', { name: '播放推荐', exact: true }).click()
  const sourceDialog = page.getByRole('dialog', { name: '需要配置自定义源' })
  await expect(sourceDialog).toContainText('请先在设置中导入并启用一个可用的自定义音源。')
  await expect(sourceDialog.getByRole('link', { name: '打开自定义源设置' })).toHaveAttribute(
    'href',
    '/settings#lx-source-settings',
  )
  expect(state.playInfoRequests).toEqual(['wy:800'])
  expect(state.mediaRequests).toBe(0)
  await expect(page.locator('.mini-player')).toContainText('未播放')
  await expect(page.locator('.discovery-feedback [role="alert"]')).toHaveCount(0)
  await sourceDialog.getByRole('button', { name: '稍后', exact: true }).click()
  await page.getByRole('button', { name: '播放推荐', exact: true }).click()
  await expect(sourceDialog).toBeVisible()
  expect(state.playInfoRequests).toEqual(['wy:800', 'wy:800'])
  expect(state.sourceRequests).toBe(0)
  await sourceDialog.getByRole('button', { name: '稍后', exact: true }).click()
...
test('无音频源仍可浏览榜单，单曲和批量预检不解析地址、不请求管理员 sources', async ({ page }) => {
  // 曲目元数据可来自旧源缓存；以已加载的 provider.play=false 为当前能力，不伪造本地缓存。
  const state = await fixture(page, { playable: false })
  await page.goto('/')
  await selectSource(page, 'wy', '网抑云')
  await expect(page.locator('.chart-panel .track-row')).toHaveCount(12)
  await expect(page.locator('.chart-directory-card')).toHaveCount(33)
  const mini = page.locator('.mini-player')
  const before = await mini.textContent()
  const providerRequests = state.providerRequests
  expect(providerRequests).toBeGreaterThan(0)

  for (const label of ['播放 精选测试榜1 前三首', '播放 精选测试榜1歌曲1']) {
    await page.getByRole('button', { name: label, exact: true }).click()
    const dialog = page.getByRole('dialog', { name: '需要配置自定义源' })
    await expect(dialog).toContainText('请先在设置中导入并启用一个可用的自定义音源。')
    await expect(dialog.getByRole('link', { name: '打开自定义源设置' })).toHaveAttribute(
      'href',
      '/settings#lx-source-settings',
    )
    await expect(mini).toHaveText(before!)
    expect(state.playInfoRequests).toEqual([])
    expect(state.mediaRequests).toBe(0)
    expect(state.sourceRequests).toBe(0)
    expect(state.providerRequests).toBe(providerRequests)
    await dialog.getByRole('button', { name: '稍后', exact: true }).click()
  }
  await mini.getByRole('button', { name: '播放队列', exact: true }).click()
  await expect(page.getByRole('dialog', { name: '播放队列 · 0', exact: true })).toContainText('播放队列为空')
})
