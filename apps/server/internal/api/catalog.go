...
func (s *Server) playInfo(w http.ResponseWriter, r *http.Request) {
	...
	var err error
	if s.live != nil {
		// 播放必须先经过本地 LX 源门禁；没有源时不能先请求目录详情。
		settings, err = s.store.Settings(r.Context())
		if err != nil {
			s.dbError(w, err)
			return
		}
		if err = s.live.RequireSourceForTrack(r.PathValue("id"), settings.AutoSwitchSource); err != nil {
			if errors.Is(err, lxsource.ErrNoActive) || errors.Is(err, lxsource.ErrUnsupported) {
