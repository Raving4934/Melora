...
// CoverImageHosts 返回封面出站策略使用的固定主机集合；调用方不得自行扩展。
func CoverImageHosts() []string { return append([]string(nil), coverImageHosts...) }
...
// CoverDownloadTarget 在 CoverTarget 的固定图片域白名单上保留受控 HTTP。
// HTTP 仅用于已支持的 CDN 主机，实际请求仍由 netguard 负责公网 DNS、逐跳重定向与响应限制。
func CoverDownloadTarget(raw string) (string, string, bool) {
	raw = strings.TrimSpace(raw)
