...
func (b *Broker) Do(ctx context.Context, request Request) (Response, error) {
	...
	for name, value := range request.Headers {
		...
		switch lower {
		case "range":
			// 仅允许显式限长的二进制文件头探测，不能借 Range 分片下载整首。
			if prefix || method != http.MethodGet || !request.Binary || request.MaxResponseBytes < 1 || responseLimit > 4096 || value != "bytes=0-"+strconv.Itoa(responseLimit-1) {
				return Response{}, ErrPolicy
			}
			prefix = true
		case "host", "connection", "upgrade", "proxy-authorization", "proxy-connection", "content-length", "transfer-encoding", "accept-encoding", "te", "trailer":
			return Response{}, ErrPolicy
		}
	...
	if method == http.MethodHead {
		return metadataResponse(resp, nil, request.Binary), nil
	}
	if !prefix && resp.ContentLength > int64(responseLimit) {
		return Response{}, ErrLimit
	}
	kind, _, _ := mime.ParseMediaType(resp.Header.Get("Content-Type"))
	if !request.Binary && (strings.HasPrefix(kind, "audio/") || strings.HasPrefix(kind, "video/") || kind == "application/ogg" || kind == "application/vnd.apple.mpegurl" || kind == "application/x-mpegurl" || kind == "application/dash+xml") {
	...
	readLimit := int64(responseLimit) + 1
	if prefix {
		if encoding != "" && encoding != "identity" {
			return Response{}, ErrPolicy
		}
		// CDN 忽略 Range 返回 200/完整长度时，也只读指定前缀并立即关流。
		readLimit = int64(responseLimit)
	}
	...
	if encoding != "" && encoding != "identity" {
		...
		plain, readErr := io.ReadAll(io.LimitReader(decoded, int64(responseLimit)+1))
		_ = decoded.Close()
		if len(plain) > responseLimit {
			return Response{}, ErrLimit
		}
		if readErr != nil {
			return Response{}, ErrPolicy
	...
	if !request.Binary && mediaBody(body) {
		return Response{}, ErrMedia
	}
	return metadataResponse(resp, body, request.Binary), nil
}
func metadataResponse(resp *http.Response, body []byte, binary bool) Response {
	headers := make(map[string]string)
	for k, v := range resp.Header {
		headers[strings.ToLower(k)] = strings.Join(v, ", ")
	}
	return Response{StatusCode: resp.StatusCode, StatusMessage: http.StatusText(resp.StatusCode), Headers: headers, Body: body, Binary: binary}
}
