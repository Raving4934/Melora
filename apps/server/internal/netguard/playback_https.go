...
func playbackAudioHEAD(resp *http.Response) bool {
	...
	case "application/octet-stream", "binary/octet-stream":
		// 部分 CDN 用通用二进制类型返回音频；只接受明确音频扩展名的
		// HTTPS HEAD 响应，仍不读正文、不放行 HTML/播放列表/无类型页面。
		if resp.Request == nil || resp.Request.URL == nil {
			return false
		}
		switch strings.ToLower(path.Ext(resp.Request.URL.Path)) {
		case ".mp3", ".aac", ".m4a", ".mp4", ".flac", ".ogg", ".opus", ".wav":
		default:
			return false
		}
	default:
