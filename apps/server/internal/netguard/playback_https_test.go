...
func TestV18HTTPSProbeOnlyConfirmsUnambiguousAudioHEAD(t *testing.T) {
	cases := []struct {
		...
		{"octet-stream-audio-path", func(r *http.Response) { r.Header.Set("Content-Type", "application/octet-stream") }, true},
...
func TestHTTPSBinaryMetadataRequiresAudioExtension(t *testing.T) {
	for _, tc := range []struct {
		path string
		ok   bool
	}{
		{"/audio.aac", true}, {"/audio.flac", true}, {"/audio.MP3", true},
		{"/audio.ogg", true}, {"/audio.m4a", true},
		{"/resolve", false}, {"/page.html", false}, {"/list.m3u8", false},
		{"/file?name=audio.aac", false},
	} {
		t.Run(tc.path, func(t *testing.T) {
			body := &v18UnreadBody{t: t}
			p := v18Probe(roundTripFunc(func(r *http.Request) (*http.Response, error) {
				resp := v18Head(r, http.StatusOK, body)
				resp.Header.Set("Content-Type", "application/octet-stream")
				return resp, nil
			}))
			_, err := p.verify(t.Context(), "http://media.example.com"+tc.path)
			if (err == nil) != tc.ok || !body.closed {
				t.Fatalf("HEAD compatibility: err=%v closed=%v", err, body.closed)
			}
		})
	}
}
