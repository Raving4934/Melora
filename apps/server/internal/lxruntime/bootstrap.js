(function (native, infoJSON) {
  ...
  globalThis.lx = {
    ...
    request(url, options, callback) {
      ...
      // 仅把受支持的通用请求字段交给 Broker；不能设置代理、agent 或 TLS 策略。
      const request = {url: String(url), method: options.method || 'GET', headers: requestHeaders(options.headers), timeout: options.timeout || 0, binary: options.binary === true, maxResponseBytes: options.maxResponseBytes === undefined ? 0 : options.maxResponseBytes};
      let data;
      let json = false;
      let body;
