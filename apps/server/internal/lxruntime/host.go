...
func (w *runtimeWorker) install() error {
	...
	_ = native.Set("request", func(call goja.FunctionCall) goja.Value {
		...
		if json.Unmarshal([]byte(text), &request) != nil || len(request.URL) > 8192 || len(request.Body) > netguard.MaxRequestBytes || request.MaxResponseBytes < 0 || request.MaxResponseBytes > netguard.MaxResponseBytes {
			w.fail(ErrLimit)
		}
		fn, ok := goja.AssertFunction(call.Argument(1))
		if !ok {
