...
func TestLiveV5CapabilitiesFollowAutoSwitch(t *testing.T) {
	l, _, _, _ := liveFixture(t, 2, map[string][]string{"// fixture 0": {"128k"}, "// fixture 1": {"320k", "flac"}})
	...
	if track.CanDownload || len(track.Qualities) != 0 {
		t.Fatal("disabled sources must not be revived by auto-switch")
	}
	if _, err := l.PlayInfo(t.Context(), fixtureTrack(), "flac"); !errors.Is(err, lxsource.ErrNoActive) {
		t.Fatalf("expected source guidance: %v", err)
	}
