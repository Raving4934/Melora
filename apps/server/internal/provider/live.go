...
var ErrMediaResult = errors.New("音源返回的媒体结果身份无效")
...
// RequireSourceForTrack 只读取曲目 ID 与已导入源状态，不访问目录或媒体网络。
// 播放/下载入口必须先调用它；元数据浏览不调用，因此没有源时仍可独立浏览目录。
func (l *Live) RequireSourceForTrack(id string, autoSwitch bool) error {
	platform, local, ok := strings.Cut(id, ":")
	if !ok || local == "" || catalog.PlatformNames[platform] == "" {
	...
	ordered := orderedSources(state, ResolveOptions{AutoSwitch: autoSwitch})
	if len(ordered) == 0 {
		return lxsource.ErrNoActive
	}
	ready := false
	for _, source := range ordered {
		...
		if len(sourceQualities(source, platform)) > 0 {
			return nil
		}
	}
	if !ready {
