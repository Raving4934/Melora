...
func cover(ctx context.Context, raw string) ([]byte, error) {
	...
	target, httpHost, ok := catalog.CoverDownloadTarget(raw)
	if !ok {
		return nil, errCoverPolicy
	}
	options := netguard.Options{AllowedRedirectHostSuffixes: catalog.CoverImageHosts()}
	if httpHost != "" {
		// 仅允许 CoverDownloadTarget 已确认的固定 CDN 主机使用 HTTP；
		// 不开启 AllowPublicHTTP，避免把任意公网 HTTP 误放进封面请求。
		options.AllowHTTPHosts = []string{httpHost}
	}
	broker, err := netguard.NewBroker(options)
	if err != nil {
		return nil, errors.New("封面请求不可用")
	}
	defer broker.Close()
	response, err := broker.Do(ctx, netguard.Request{URL: target, Method: http.MethodGet, Headers: map[string]string{"Accept": "image/jpeg,image/png"}, Timeout: 5000})
