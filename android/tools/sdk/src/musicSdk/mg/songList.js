...
const queryId = (query, key) => new RegExp(`(?:^|&)${key}=(\\d+)(?:&|$)`).exec(query)?.[1] || null
...
const parsePlaylistId = value => {
  ...
  if (url.pathname.endsWith('/playlist/index.html')) return queryId(url.query, 'id')

  const queryAt = url.hash.indexOf('?')
  const hashPath = queryAt < 0 ? url.hash : url.hash.slice(0, queryAt)
  if (/^#?\/?playlist\/?$/.test(hashPath)) {
    return queryId(queryAt < 0 ? '' : url.hash.slice(queryAt + 1), 'playlistId')
  }
  return null
}
...
export default {
  ...
  async getDetailUrl(link, page) {
    ...
    for (let hop = 0; hop < 5; hop++) {
      ...
      const playlistId = parsePlaylistId(location)
      if (playlistId) {
        // 只记最终歌单编号；分页复用，不能把中间短链存回缓存形成循环。
  ...
  getListDetail(id, page, retryNum = 0) { // 获取歌曲列表内的音乐
    ...
    if (!/^\d+$/.test(inputId)) {
      id = parsePlaylistId(inputId) || this.resolvedPlaylistIds.get(inputId)
  ...
  getDetailPageUrl(id) {
    id = parsePlaylistId(String(id)) || id
