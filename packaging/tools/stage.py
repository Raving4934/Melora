...
def validate_icons(repo, template):
    contract = json.loads((template / 'icon-contract.json').read_text())
    source = contract.get('source')
    require(source == 'apps/web/src/assets/brand/app-icon.svg', '图标契约的正式 SVG 路径无效。')
    source_path = repo / source
    require(source_path.is_file() and not source_path.is_symlink(), '正式图标 SVG 缺失或为符号链接。')
    digest = lambda path: hashlib.sha256(path.read_bytes()).hexdigest()
    require(contract.get('sourceSha256') == digest(source_path), '正式图标 SVG 已变化，请重新生成 FPK 图标。')
    icons = contract.get('icons')
    require(isinstance(icons, dict) and set(icons) == {
        'ICON.PNG', 'ICON_256.PNG', 'app/ui/images/icon_64.png', 'app/ui/images/icon_256.png'
    }, '图标契约文件清单无效。')
    for name, expected in icons.items():
        path = template / name
        require(path.is_file() and not path.is_symlink() and digest(path) == expected,
                f'FPK 图标未与正式 SVG 同步：{name}')
...
def stage(repo, dest, arch, epoch):
    ...
    require(json.loads((dest / 'config/privilege').read_text()).get('defaults', {}).get('run-as') == 'root',
            '生命周期必须使用 root，并由 cmd/main 降权启动 package 服务。')
