...
ICON_GENERATOR = PACKAGING / 'tools/generate-icons.py'
...
class PackageTests(unittest.TestCase):
    def setUp(self):
        ...
        for name in ['scripts', 'dist', 'apps/web/dist', 'apps/web/src/assets/brand', 'packaging']:
        ...
        shutil.copy2(REPO / 'apps/web/src/assets/brand/app-icon.svg',
                     self.root / 'apps/web/src/assets/brand/app-icon.svg')
    ...
    def test_both_architectures_and_exact_structure(self):
        for arch, platform in [('amd64', 'x86'), ('arm64', 'arm')]:
            ...
            self.assertEqual(json.loads((stage / 'config/privilege').read_text())['defaults']['run-as'], 'root')
    ...
    def test_stage_rejects_icons_not_matching_canonical_contract(self):
        for name in ('ICON.PNG', 'ICON_256.PNG', 'app/ui/images/icon_64.png', 'app/ui/images/icon_256.png'):
            (self.root / 'packaging/fpk' / name).write_bytes(b'STALE ICON MUST NOT SHIP')
        result = self.run_package('--arch', 'amd64', '--stage-only', expected=1)
        self.assertIn('FPK 图标未与正式 SVG 同步', result.stderr)
        self.assert_no_fpk()
...
class IconGeneratorTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        spec = importlib.util.spec_from_file_location('melora_generate_icons', ICON_GENERATOR)
    ...
    def decode_rgba_png(data):
        if not data.startswith(b'\x89PNG\r\n\x1a\n'):
            raise AssertionError('不是 PNG 文件')
        offset = 8
        idat = bytearray()
        width = height = None
        while offset < len(data):
            ...
            if kind == b'IHDR':
                ...
                if (depth, color_type, compression, filtering, interlace) != (8, 6, 0, 0, 0):
                    raise AssertionError('PNG 不是 8-bit RGBA 非交错格式')
            elif kind == b'IDAT':
                idat.extend(payload)
            elif kind == b'IEND':
                break
        if width is None or height is None:
            raise AssertionError('PNG 缺少 IHDR')
        row_bytes = width * 4
        raw = zlib.decompress(bytes(idat))
        if len(raw) != height * (row_bytes + 1):
            raise AssertionError('PNG 解码长度错误')
        rows = []
        for row in range(height):
            start = row * (row_bytes + 1)
            if raw[start] != 0:
                raise AssertionError('测试只接受生成器的 filter 0 PNG')
            rows.append(raw[start + 1:start + row_bytes + 1])
        return width, height, [tuple(row[index:index + 4])
                              for row in rows for index in range(0, row_bytes, 4)]

    def test_renderer_uses_formal_android_web_icon_contract(self):
        formal_icon = (REPO / 'apps/web/src/assets/brand/app-icon.svg').resolve()
        android_logo = (REPO / 'android/app/src/main/res/drawable/ic_melora_logo.xml').read_text()
        android_wave = (REPO / 'android/app/src/main/res/drawable/ic_melora_wave.xml').read_text()

        self.assertEqual(self.generator.FORMAL_ICON.resolve(), formal_icon)
        self.assertTrue(formal_icon.is_file())
        self.assertIn('fill="#5B89FA"', formal_icon.read_text())
        self.assertNotIn('#03A550', formal_icon.read_text())
        self.assertIn('android:fillColor="#5B89FA"', android_logo)
        self.assertIn('android:fillColor="#FFFFFF"', android_logo)
        self.assertIn('android:fillColor="#FFFFFF"', android_wave)
        self.assertEqual(self.generator.BLUE, (91, 137, 250, 255))
        self.assertEqual(self.generator.WHITE, (255, 255, 255, 255))

    def test_committed_icons_match_renderer_and_pair_copies(self):
        assets = {
            64: [PACKAGING / 'fpk/ICON.PNG', PACKAGING / 'fpk/app/ui/images/icon_64.png'],
            256: [PACKAGING / 'fpk/ICON_256.PNG', PACKAGING / 'fpk/app/ui/images/icon_256.png'],
        }
        for size, paths in assets.items():
            generated = self.generator.render(size)
            for path in paths:
                self.assertEqual(path.read_bytes(), generated, path)
            self.assertEqual(paths[0].read_bytes(), paths[1].read_bytes())

    def test_icon_generation_is_idempotent_and_semantically_blue_white(self):
        with tempfile.TemporaryDirectory(prefix='icons-', dir=BUILD) as folder:
            root = Path(folder) / 'fpk'
            root.mkdir()
            self.generator.generate(root)
            paths = [root / 'ICON.PNG', root / 'ICON_256.PNG', root / 'icon-contract.json',
                     root / 'app/ui/images/icon_64.png', root / 'app/ui/images/icon_256.png']
            first = {path.relative_to(root): path.read_bytes() for path in paths}
            self.generator.generate(root)
            second = {path.relative_to(root): path.read_bytes() for path in paths}
            self.assertEqual(first, second)

            blue = (91, 137, 250, 255)
            white = (255, 255, 255, 255)
            for size in (64, 256):
                icon = root / ('ICON.PNG' if size == 64 else 'ICON_256.PNG')
                width, height, pixels = self.decode_rgba_png(icon.read_bytes())
                scale = size // 64
                self.assertEqual((width, height), (size, size))
                self.assertIn(blue, pixels)
                self.assertIn(white, pixels)
                self.assertEqual(pixels[(2 * scale) * size + 32 * scale], blue)
                self.assertEqual(pixels[(38 * scale) * size + 44 * scale], white)
                self.assertNotIn((53, 103, 232, 255), pixels)
                self.assertNotIn((28, 83, 68, 255), pixels)
                self.assertNotIn((3, 165, 80, 255), pixels)
                self.assertFalse(
                    any(alpha and green > red * 1.35 and green > blue_channel * 1.1
                        for red, green, blue_channel, alpha in pixels),
                    f'{size}px 图标不得包含旧绿色底板',
                )
                self.assertTrue(
                    any(0 < alpha < 255 for _, _, _, alpha in pixels),
                    f'{size}px 图标必须保留抗锯齿透明边缘',
                )
                for red, green, blue_channel, alpha in pixels:
                    if alpha == 0:
                        self.assertEqual((red, green, blue_channel), (0, 0, 0))
                    elif alpha < 255:
                        self.assertEqual((red, green, blue_channel), (91, 137, 250))
